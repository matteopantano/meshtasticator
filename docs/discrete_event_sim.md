...
After the simulations are done, it plots relevant metrics obtained from the simulations. It saves these metrics in `out/report/` to analyze them later on. See [`plotExample.py`](../plotExample.py) for an example Python script to plot the results.
